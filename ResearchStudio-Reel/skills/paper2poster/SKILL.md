...
- **Subsection headings** in both `methoddriven` and `methoddriven4` use outlined square **A, B, C, D…** badges (`.msub-h .n`). Keep numeric steps inside cards separate; do not turn heading badges back into filled numbered circles. When adding/reordering cards, continue the letters in reading order while preserving `data-section="method-1"…"method-N"` for measurement and Reel interactions.
